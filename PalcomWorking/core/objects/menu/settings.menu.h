...
class SettingsMenu : public PalcomMenu{
	private:
		...
		PalcomObject rootPg;
	public:
		void make(lv_obj_t *parent, void (*func)(lv_event_t *)){
    ...
    lv_obj_set_size(menu, this->getHorizontalResolution(), this->getVirticalResolution());
    lv_obj_center(menu);
    ...
    lv_obj_send_event(lv_obj_get_child(lv_obj_get_child(lv_menu_get_cur_sidebar_page(menu), 0), 0), LV_EVENT_CLICKED,NULL);
			...
			/*
                        lv_color_t color = this->getStyleBgColor(0);
                        if(this->colors.getColorBrightness(color) > 127){
                                this->setStyleBgColor(this->colors.darken(color, 10), 0);
                        }else{
                                this->setStyleBgColor(this->colors.darken(color, 50), 0);
			 ...
			 * */
                        ...
                        //this->setParamCallback(func, (void *)this->getObject());
                        //this->setSize(this->getHorizontalResolution(), this->getVirticalResolution());
                        //this->center();
			this->setAlignment(LV_ALIGN_OUT_TOP_RIGHT, 0, 0);
			this->setSize(200, 300);

			backBtn.setObject(this->getMainBackButton());
			backBtn.setSimpleCallback(func);


			/*
			 * Setup the root page.
			 * */
		/*	this->rootPage = this->newPage("Settings");
			lv_obj_set_style_pad_hor(rootPage, lv_obj_get_style_pad_left(lv_menu_get_main_header(this->getObject()), 0), 0);

*/
			//rootPg.setObject(this->rootPage);

			//rootPg.setStylePaddingHor(this->getMainHeaderLeftPadding(), 0);
			//
		}
};

...
lv_color_t SetMsgModeButtonStyle::darken(const lv_color_filter_dsc_t * dsc, lv_color_t color, lv_opa_t opa){
        LV_UNUSED(dsc);
	return lv_color_darken(color, opa);
}

lv_style_t *SetMsgModeButtonStyle::getStyle(void){
	return (lv_style_t *)&submitButton;
}

lv_style_t *SetMsgModeButtonStyle::getPressedStyle(void){
	return (lv_style_t *)&submitButtonPressed;
}

lv_color_filter_dsc_t *SetMsgModeButtonStyle::getColorFilter(void){
	return (lv_color_filter_dsc_t *)&color_filter;
}
void SetMsgModeButtonStyle::initStyle(){
